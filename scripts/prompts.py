...
- Keep the research lightweight: total external sources across the whole run must be <= 4 (prefer 2–3).
...
SCORING (0–1000)
- Start from the provided BrainScore (0–1000). Treat it as the primary signal.
- Apply a conservative adjustment based on EV/risk discovered during your lookups.
  - Typical range: -10% to +8% (relative). Prefer small adjustments.
  - If you apply a larger negative adjustment (>-10%), you must cite a very strong reason (e.g., fraud probe, severe going concern).
...
USER_PROMPT_TOP20_TEMPLATE = (
    "TODAY is {today}. You are given the Top-10 ML-ranked candidates (already the best of the day), ordered by BrainScore.\n"
    "You must pick exactly ONE winner with the best EV and lowest risk, using up-to-date web data.\n\n"
