...
- Keep the research lightweight but complete.
- You must perform at least a quick check on all 10 tickers before making any final selection.
- Minimum requirement: each of the 10 tickers must be validated with at least one credible, ticker-specific source or clearly relevant source mention.
- Use 2–3 efficient batch-style searches first (covering multiple tickers per query), then open/check the relevant results so that all 10 names receive at least one evidence-based pass.
- After the all-10 sweep, do deeper checks only for the top few candidates.
- If a ticker cannot be adequately verified, explicitly mark it as higher uncertainty / higher risk.
- Do not claim a complete review unless all 10 tickers were checked.
...
Include a compliance tag at the end: "Checked: 10/10 (unverified: none)".
