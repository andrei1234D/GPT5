...
def pick_top_stratified(
    ...
    The function preserves score order as much as possible, and falls back
    gracefully when constraints are infeasible.
    """
    if total <= 0:
        return []
    ...
    # Fill the rest by best-available, with bias towards meeting pe_min
    while len(picked) < total:
        ...
        if need_pe > 0:
            ...
            if next_with_pe is not None:
                ...
                continue
        # Otherwise, take next best overall
        nxt = remaining[0]
        picked.append(nxt)
        picked_ids.add(nxt[0])

    # If we couldn't hit pe_min yet, try PE-improving swaps (keep quotas)
    pe_have = count_pe(picked)
    if pe_have < pe_min:
        # candidates to insert (best first) with PE and not already picked
        insertables = [r for r in ranked if (r[0] not in picked_ids) and _has_pe(r)]

        # removal candidates = lowest-scoring picks WITHOUT PE,
        # but only if removing them won't break min_small/min_large
        def tier_counts(rows):
            s = sum(1 for r in rows if _tier_of_row(r) == "small")
            l = sum(1 for r in rows if _tier_of_row(r) == "large")
            return s, l

        picks_sorted_lo = sorted(picked, key=lambda x: x[3])  # ascending by score
        ins_idx = 0
        while (pe_have < pe_min) and (ins_idx < len(insertables)):
            incoming = insertables[ins_idx]
            ins_idx += 1
            removed_any = False
            for j, cand_out in enumerate(picks_sorted_lo):
                if _has_pe(cand_out):
                    continue
                # simulate removal
                ...
                if (s_cnt >= min_small) and (l_cnt >= min_large):
                    ...
                    # rebuild low-score list
                    picks_sorted_lo = sorted(picked, key=lambda x: x[3])
                    pe_have = count_pe(picked)
                    removed_any = True
                    break
            if not removed_any:
                continue  # can't remove anyone without violating quotas

    # Final sort by score desc and trim
